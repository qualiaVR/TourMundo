Shader "Hidden/DigitalSalmon/C360/MediaNodeBackground" {
    Properties {
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
				...
				float4 _MainTex_var = BlurredTexSample(_MainTex, tinyPlanetUV, 1, 6);
                float radialField = RadialField( uv );
                float tinyPlanetMask = TinyPlanetMask( radialField );
                float ringMask = RingMask( radialField );
     
                float3 color = lerp(0, lerp(_MainTex_var.rgb,0, InnerShadowMask(radialField) * _InnerShadow), tinyPlanetMask);
