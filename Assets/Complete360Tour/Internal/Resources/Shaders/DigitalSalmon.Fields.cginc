...
#include "Assets/Complete360Tour/Internal/Resources/Shaders/DigitalSalmon.Math.cginc"
...
float smoothMultiplierFromDistance(float distance) {
...
float udRadial(float2 domain) {
	return (atan2(-domain.x, -domain.y) / 6.28318530718) + 0.5 ;
}
...
float sdHex(float2 domain, float radius) {
	const float asp = 0.8660254037844386;
	float2 q = abs(domain);
	return max((q.x*asp + q.y*0.5), q.y) - radius + ((1 - asp) / 2);
}
...
float sdTri(float2 domain, float radius) {
	float2 q = abs(domain);
	return max(q.x*0.866025 + domain.y*0.5, -domain.y) - radius*0.5;
}

// 3D TriangularPrism
float sdTriPrism(float3 domain, float radius, float height) {
	float3 q = abs(domain);
	return max(q.z - height, max(q.x*0.866025 + domain.y*0.5, -domain.y) - radius*0.5);
}
