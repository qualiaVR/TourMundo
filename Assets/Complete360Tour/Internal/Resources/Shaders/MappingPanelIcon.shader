Shader "Hidden/DigitalSalmon/C360/MappingPanelIcon" {
    Properties {
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
				...
				float4 col =  tex2D(_MainTex, clamp(ScaleUV(uv, 2), 0, 1)) ;
				col.rgb = lerp(col.rgb, _Highlight, col.a);
				
				col.a = max(col.a, DropShadowMask(radialField) * 0.3);
				
				col.a = min(col.a, 1- RingMask(radialField) );
			
				col.a = max(col.a, RingMask(radialField) * _Highlight.a);	
